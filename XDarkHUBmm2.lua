...
local function ban(msg, col)
    pcall(function()
        if not _banGui then
            ...
            f.Size = UDim2.new(0, 660, 0, 150)
            f.Position = UDim2.new(0.5, -330, 0, 14)
            f.BackgroundColor3 = Color3.fromRGB(10, 10, 12)
            f.BorderSizePixel = 0
            pcall(function() Instance.new("UICorner", f).CornerRadius = UDim.new(0, 12) end)
            pcall(function()
                local s = Instance.new("UIStroke", f)
                s.Color = Color3.fromRGB(224, 49, 62)
                s.Thickness = 2
            end)
            _banTxt = Instance.new("TextLabel", f)
            _banTxt.Size = UDim2.new(1, -24, 1, -24)
            _banTxt.Position = UDim2.new(0, 12, 0, 12)
            _banTxt.BackgroundTransparency = 1
            _banTxt.Font = Enum.Font.GothamBold
            _banTxt.TextSize = 16
            _banTxt.TextWrapped = true
            _banTxt.TextXAlignment = Enum.TextXAlignment.Left
            _banTxt.TextYAlignment = Enum.TextYAlignment.Top
            _banTxt.ZIndex = 5
        end
        _banTxt.Text = msg
        _banTxt.TextColor3 = col or Color3.fromRGB(245, 245, 248)
    end)
end
...
xpcall(function()
    ...
    local COL = {
        bgDeep = Color3.fromRGB(11, 11, 14),
        bg = Color3.fromRGB(17, 17, 21),
        panel = Color3.fromRGB(23, 23, 28),
        card = Color3.fromRGB(31, 31, 37),
        cardHover = Color3.fromRGB(42, 42, 49),
        accent = Color3.fromRGB(224, 49, 62),
        accentHot = Color3.fromRGB(244, 86, 98),
        accentDim = Color3.fromRGB(104, 24, 32),
        ...
        track = Color3.fromRGB(52, 52, 59),
        line = Color3.fromRGB(34, 34, 40),
        knob = Color3.fromRGB(246, 246, 250),
    }
    ...
    local function notify(title, text, duration)
        ...
        if toastHolder and toastHolder.Parent then
            handled = pcall(function()
                ...
                t.BackgroundColor3 = Color3.fromRGB(26, 26, 31)
                ...
                bar.BackgroundColor3 = COL.accentHot
    ...
    local function readRoleFromTable(tbl)
        ...
        return normalizeRoleName(
            tbl.Role or tbl.role or tbl.RoleName or tbl.rolename or
            tbl.Status or tbl.status or tbl.Team or tbl.team or
    ...
    local function getPlayerRole(pl)
        ...
        local function hasTool(n)
            if pl.Backpack and pl.Backpack:FindFirstChild(n) then return true end
            if pl.Character and pl.Character:FindFirstChild(n) then return true end
            return false
        end
    ...
    local function getPredictedPosition(tp)
        ...
        local phrp = char:FindFirstChild("UpperTorso") or char:FindFirstChild("HumanoidRootPart")
        ...
        pred = pred * ((ping * ((offsetToPingMult - 1) * 0.01)) + 1)
        return pred
    end
    ...
    function miniFling(playerToFling)
        ...
        local SFBasePart = function(bp)
            ...
            until bp.Velocity.Magnitude > 500
                or bp.Parent ~= playerToFling.Character
                or playerToFling.Parent ~= Players
    ...
    refreshESP = function()
        ...
        for pl in pairs(espObjects) do
            if not alive[pl] then clearPlayerHighlight(pl) end
        end
    end
    ...
    local function reloadGunESP()
        ...
        if map and map:FindFirstChild("GunDrop") then
            gunHighlight = newHighlight({
                ...
                OutlineTransparency = 0,
                DepthMode = Enum.HighlightDepthMode.AlwaysOnTop,
                Adornee = map:FindFirstChild("GunDrop")
            })
    ...
    function shootMurderer()
        ...
        xdSpawn(function()
            for shot = 1, 3 do
                ...
                local aim = instakillshoot and (mhrp.Position + Vector3.new(0, 1, 0)) or predicted
                local rh = localplayer.Character:FindFirstChild("RightHand")
                local origin = rh and rh.Position or localplayer.Character:GetPivot().Position
                pcall(function() gun:WaitForChild("Shoot"):FireServer(CFrame.new(origin), CFrame.new(aim)) end)
                xdWait(0.12)
            end
    ...
    function knifeThrow()
        ...
        if not localplayer.Character:FindFirstChild("Knife") then
            ...
            if localplayer.Backpack:FindFirstChild("Knife") then
                ...
                notify(HUB_NAME, "У тебя нет ножа."); return
            end
        end
        ...
        pcall(function()
            localplayer.Character:WaitForChild("Knife"):WaitForChild("Events"):WaitForChild("KnifeThrown"):FireServer(unpack(args))
        end)
    ...
    function killClosest()
        ...
        if not nhrp or not myHRP then return end
        nhrp.Anchored = true
        nhrp.CFrame = myHRP.CFrame + myHRP.CFrame.LookVector * 2
        xdWait(0.1)
    ...
    workspace.DescendantAdded:Connect(function(ch)
        pcall(function()
            if trapDetection and ch.Name == "Trap" and ch.Parent and (ch.Parent:IsA("Folder") or ch.Parent:IsA("Model")) then
                ...
                reloadTrapESP()
                notify(HUB_NAME, "Убийца поставил ловушку!")
            end
            if gunDropESP and ch.Name == "GunDrop" then
                reloadGunESP()
                notify(HUB_NAME, "Пистолет выпал!")
    ...
    -- ================= ВИЗУАЛЫ: МЯГКИЙ BLOOM КАК НА ФОТО =================
    local visualState = {
        wings = true, circle = true, halo = true,
        aura = false, fire = false, smoke = false,
        trails = false, eyes = false, light = false, lightning = false,
    }
    local visualObjects = {}
    local wingParts = {}
    local wingMats = {}
    ...
    local floorWaves = {}
    local floorPool = nil
    local floorPoolGlow = nil
    local eyeParts = {}

    local CORE = Color3.fromRGB(255, 222, 226)
    local GLOW = Color3.fromRGB(244, 52, 66)
    ...
    local function clearVisual(name)
        ...
        if name == "wings" then wingParts = {}; wingMats = {} end
        if name == "halo" then haloBeads = {}; haloRoot = nil end
        if name == "circle" then floorWaves = {}; floorPool = nil; floorPoolGlow = nil end
    ...
    local function glowOf(src, scale, transp, color)
        local g = nil
        pcall(function()
            g = src:Clone()
            g.Name = src.Name .. "_g"
            g.Transparency = transp or 0.55
            if color then g.Color = color end
            if g:FindFirstChildOfClass("SpecialMesh") then
                g:FindFirstChildOfClass("SpecialMesh").Scale = src:FindFirstChildOfClass("SpecialMesh").Scale * (scale or 1.5)
            else
                g.Size = src.Size * (scale or 1.5)
            end
            g.Parent = src.Parent
        end)
        return g
    ...
    local function applyWings()
        ...
        for side = -1, 1, 2 do
            local mat = neon({Name = "XWingMat", Color = CORE, Transparency = 0.32, Parent = char}, Vector3.new(0.55, 3.4, 4.2))
            local matG = glowOf(mat, 1.35, 0.7, GLOW)
            registerVisual("wings", mat)
            if matG then registerVisual("wings", matG) end
            table.insert(wingMats, {part = mat, glow = matG, side = side})
            local count = 8
            for i = 1, count do
                local t = (i - 1) / (count - 1)
                local len = 3.6 - math.abs(t - 0.42) * 2.2
                local wid = 1.5 - t * 0.5
                local feather = neon({Name = "XWingF", Color = CORE:lerp(GLOW, t * 0.3), Transparency = 0.03, Parent = char}, Vector3.new(0.5, len, wid))
                local fG = glowOf(feather, 1.5, 0.6, GLOW)
                registerVisual("wings", feather)
                if fG then registerVisual("wings", fG) end
                table.insert(wingParts, {
                    part = feather, glow = fG, side = side, i = i,
                    len = len, baseAng = 10 + t * 78, flapAmp = 5 + t * 14,
                })
            end
        end
        local att = Instance.new("Attachment", hrp)
        att.Position = Vector3.new(0, 1.1, 0.3)
        registerVisual("wings", att)
        local em = Instance.new("ParticleEmitter", att)
        em.Texture = "rbxasset://textures/particles/sparkles_main.dds"
        em.Color = ColorSequence.new(CORE, GLOW)
        em.Rate = 42
        em.Lifetime = NumberRange.new(0.6, 1.3)
        em.Speed = NumberRange.new(1, 3)
        em.SpreadAngle = Vector2.new(180, 180)
        em.LightEmission = 1
        em.Size = NumberSequence.new({NumberSequenceKeypoint.new(0, 0.42), NumberSequenceKeypoint.new(1, 0)})
        registerVisual("wings", em)
        local wl = Instance.new("PointLight")
        wl.Color = Color3.fromRGB(255, 80, 95)
        wl.Brightness = 3.4
        wl.Range = 28
        wl.Parent = hrp
        registerVisual("wings", wl)
    end
    ...
    local function applyHalo()
        ...
        haloRoot = Instance.new("Part")
        haloRoot.Name = "XHaloRoot"
        haloRoot.Transparency = 1
        haloRoot.Anchored = true
        haloRoot.CanCollide = false
        haloRoot.Size = Vector3.new(0.1, 0.1, 0.1)
        haloRoot.Parent = char
        registerVisual("halo", haloRoot)
        local beads = 32
        local rad = 1.55
        for k = 1, beads do
            local a = (k / beads) * math.pi * 2
            local bead = neon({Name = "XHaloBead", Color = CORE, Transparency = 0.0, Parent = char}, Vector3.new(0.36, 0.36, 0.36))
            registerVisual("halo", bead)
            table.insert(haloBeads, {part = bead, a = a, rad = rad})
        end
        local hl = Instance.new("PointLight")
        hl.Color = Color3.fromRGB(255, 80, 95)
        hl.Brightness = 2.8
        hl.Range = 15
        hl.Parent = head
    ...
    local function applyCircle()
        ...
        local hrp = char and char:FindFirstChild("HumanoidRootPart")
        if not hrp then return end
        floorPool = neon({Name = "XPool", Shape = Enum.PartType.Cylinder, Size = Vector3.new(0.1, 9, 9), Color = GLOW, Transparency = 0.82, Parent = char})
        registerVisual("circle", floorPool)
        floorPoolGlow = glowOf(floorPool, 1.12, 0.9, GLOW)
        if floorPoolGlow then registerVisual("circle", floorPoolGlow) end
        for i = 1, 4 do
            local disc = neon({Name = "XWave", Shape = Enum.PartType.Cylinder, Size = Vector3.new(0.12, 3, 3), Color = CORE, Transparency = 0.4, Parent = char})
            local dG = glowOf(disc, 1.1, 0.62, GLOW)
            registerVisual("circle", disc)
            if dG then registerVisual("circle", dG) end
            table.insert(floorWaves, {part = disc, glow = dG, phase = (i - 1) / 4})
        end
        local cl = Instance.new("PointLight")
        cl.Color = Color3.fromRGB(255, 70, 85)
        cl.Brightness = 3.0
        cl.Range = 24
        cl.Parent = hrp
    ...
    local function applyEyes()
        ...
        if not head then return end
        for side = -1, 1, 2 do
            local eye = neon({Name = "XEye", Size = Vector3.new(0.12, 0.14, 0.14), Color = GLOW, Transparency = 0, Parent = char})
    ...
    local function applyLight()
        ...
        l.Color = Color3.fromRGB(255, 40, 60)
        l.Brightness = 2.2
        l.Range = 20
        l.Parent = hrp
    ...
    local function applyLightning()
        ...
        local em = Instance.new("ParticleEmitter", att)
        em.Texture = "rbxasset://textures/particles/sparkles_main.dds"
        em.Color = ColorSequence.new(Color3.fromRGB(255, 220, 180), GLOW)
        em.Rate = 70
        em.Lifetime = NumberRange.new(0.08, 0.25)
    ...
    RunService.Heartbeat:Connect(function()
        pcall(function()
            ...
            if visualState.wings and hrp then
                ...
                for _, f in ipairs(wingParts) do
                    if f.part.Parent then
                        local a = f.baseAng + flap * f.flapAmp
                        local cf = hrp.CFrame
                            * CFrame.new(f.side * 0.35, 1.2 + bob, -0.4)
                            * CFrame.Angles(0, 0, math.rad(f.side * a))
                            * CFrame.new(0, f.len / 2, 0)
                        f.part.CFrame = cf
                        if f.glow and f.glow.Parent then f.glow.CFrame = cf end
                    end
                end
                for _, m in ipairs(wingMats) do
                    if m.part.Parent then
                        local spread = 30 + flap * 10
                        local cf = hrp.CFrame
                            * CFrame.new(m.side * 1.05, 1.15 + bob, 0.9)
                            * CFrame.Angles(0, 0, math.rad(m.side * spread))
                            * CFrame.Angles(0, math.rad(m.side * -8), 0)
                        m.part.CFrame = cf
                        if m.glow and m.glow.Parent then m.glow.CFrame = cf end
                    end
                end
            end
            if visualState.halo and haloRoot and haloRoot.Parent then
                local head = char:FindFirstChild("Head")
                if head then
                    local bob = math.sin(t * 2.2) * 0.1
                    local rootCF = head.CFrame
                        * CFrame.new(0, 1.95 + bob, 0)
                        * CFrame.Angles(math.rad(70), t * 1.2, math.rad(10))
                    haloRoot.CFrame = rootCF
                    for _, b in ipairs(haloBeads) do
                        if b.part.Parent then
                            local lx = math.cos(b.a) * b.rad
                            local lz = math.sin(b.a) * b.rad
                            b.part.CFrame = rootCF * CFrame.new(lx, 0, lz)
                        end
                    end
                end
            end
            if visualState.circle and hrp then
                ...
                if floorPool and floorPool.Parent then
                    floorPool.CFrame = poolCF
                    floorPool.Transparency = 0.8 + math.sin(t * 1.5) * 0.05
                end
                ...
                for _, w in ipairs(floorWaves) do
                    if w.part.Parent then
                        local ph = ((t * 0.35 + w.phase) % 1)
                        local r = 1.4 + ph * 4.2
                        local tr = 0.25 + ph * 0.65
                        local cf = CFrame.new(cx, cy + 0.01, cz) * CFrame.Angles(math.rad(90), 0, t * 0.4)
                        w.part.CFrame = cf
                        w.part.Size = Vector3.new(0.12, r * 2, r * 2)
                        w.part.Transparency = tr
                        if w.glow and w.glow.Parent then
                            w.glow.CFrame = cf
                            w.glow.Size = w.part.Size * 1.1
                            w.glow.Transparency = math.min(0.95, tr + 0.2)
                        end
                    end
                end
            end
    ...
    -- ================= GUI: ПЛОСКИЙ SHITARO-СТИЛЬ, ДВЕ КОЛОНКИ, ЖИВОЙ =================
    ...
    local guiOK, guiERR = pcall(function()
        ...
        local guiW = clamp(viewport.X * 0.72, 540, 780)
        local guiH = clamp(viewport.Y * 0.74, 380, 540)
        pcall(function()
        ...
        local guiScale = Instance.new("UIScale", guiUI)
        guiScale.Scale = 0.92
        ...
        frame.Parent = guiUI
        corner(frame, 14)
        stroke(frame, COL.border, 1, 0)

        local function softGlow(px, py, sz, col, tr)
            local b = Instance.new("Frame")
            b.Size = UDim2.new(0, sz, 0, sz)
            b.Position = UDim2.new(px, -sz / 2, py, -sz / 2)
            b.BackgroundColor3 = col
            b.BackgroundTransparency = tr or 0.9
            b.BorderSizePixel = 0
            b.ZIndex = 5
            b.Parent = frame
            corner(b, sz / 2)
        ...
        softGlow(0.08, 0.06, 260, COL.accent, 0.93)
        softGlow(0.95, 0.97, 220, COL.ember, 0.94)
        for _, top in ipairs({true, false}) do
            local sh = Instance.new("Frame")
            sh.Size = UDim2.new(1, 0, 0, 60)
            sh.Position = top and UDim2.new(0, 0, 0, 0) or UDim2.new(0, 0, 1, -60)
            ...
            pcall(function()
                ...
                if top then
                    g.Transparency = NumberSequence.new({NumberSequenceKeypoint.new(0, 0.5), NumberSequenceKeypoint.new(1, 1)})
                else
                    g.Transparency = NumberSequence.new({NumberSequenceKeypoint.new(0, 1), NumberSequenceKeypoint.new(1, 0.5)})
                end
            end)
        end
        for i = 1, 8 do
            ...
            dot.Position = UDim2.new(math.random(18, 82) / 100, 0, math.random(20, 90) / 100, 0)
            ...
            xdSpawn(function()
                while dot.Parent do
                    ...
                    tween(
                        dot,
                        {
                            Position = UDim2.new(dot.Position.X.Scale + math.random(-10, 10) / 100, 0, dot.Position.Y.Scale + math.random(-12, 8) / 100, 0),
                            BackgroundTransparency = math.random(82, 96) / 100,
                        },
                        dur,
        ...
        ava.Size = UDim2.new(0, 36, 0, 36)
        ava.Position = UDim2.new(0, 14, 0.5, -18)
        ava.BorderSizePixel = 0
        ava.ZIndex = 9
        ava.Parent = topBar
        corner(ava, 9)
        gradient(ava, {ColorSequenceKeypoint.new(0, COL.accentHot), ColorSequenceKeypoint.new(1, COL.accentDim)}, 45)
        local avaTxt = Instance.new("TextLabel")
        avaTxt.Size = UDim2.new(1, 0, 1, 0)
        avaTxt.BackgroundTransparency = 1
        avaTxt.Text = string.upper(string.sub(HUB_NAME, 1, 1))
        avaTxt.Font = Enum.Font.GothamBlack
        avaTxt.TextSize = 19
        avaTxt.TextColor3 = COL.knob
        avaTxt.ZIndex = 10
        avaTxt.Parent = ava

        local nameT = Instance.new("TextLabel")
        nameT.Size = UDim2.new(0, 200, 0, 19)
        nameT.Position = UDim2.new(0, 60, 0, 10)
        nameT.BackgroundTransparency = 1
        nameT.Text = HUB_NAME
        nameT.Font = Enum.Font.GothamBlack
        nameT.TextSize = 16
        nameT.TextColor3 = COL.text
        nameT.TextXAlignment = Enum.TextXAlignment.Left
        nameT.ZIndex = 9
        nameT.Parent = topBar
        local subT = Instance.new("TextLabel")
        subT.Size = UDim2.new(0, 200, 0, 14)
        subT.Position = UDim2.new(0, 60, 0, 29)
        subT.BackgroundTransparency = 1
        subT.Text = "v42  ·  never"
        subT.Font = Enum.Font.GothamMedium
        subT.TextSize = 11
        subT.TextColor3 = COL.textDim
        subT.TextXAlignment = Enum.TextXAlignment.Left
        subT.ZIndex = 9
        subT.Parent = topBar

        -- живой эквалайзер рядом с именем
        local eqBox = Instance.new("Frame")
        eqBox.Size = UDim2.new(0, 22, 0, 14)
        eqBox.Position = UDim2.new(0, 60, 0, 30)
        ...
        for bi = 1, 4 do
            ...
            bar.Position = UDim2.new(0, (bi - 1) * 5, 1, 0)
            ...
            xdSpawn(function()
                local phase = bi * 0.7
                while bar.Parent do
                    local h = 0.3 + (math.sin(tick() * 6 + phase) + 1) * 0.35
                    tween(bar, {Size = UDim2.new(0, 3, h, 0)}, 0.18, Enum.EasingStyle.Sine, Enum.EasingDirection.Out)
                    xdWait(0.2)
                end
            end)
        end
        ...
        dd.Size = UDim2.new(0, 92, 0, 28)
        dd.Position = UDim2.new(1, -138, 0.5, -14)
        dd.BackgroundColor3 = COL.card
        dd.BorderSizePixel = 0
        dd.Text = "  legit    ⌄"
        dd.Font = Enum.Font.GothamBold
        dd.TextSize = 12
        dd.TextColor3 = COL.text
        dd.AutoButtonColor = false
        dd.ZIndex = 9
        dd.Parent = topBar
        corner(dd, 8)
        stroke(dd, COL.border, 1, 0.4)
        local ddIco = Instance.new("TextLabel")
        ddIco.Size = UDim2.new(0, 16, 1, 0)
        ddIco.Position = UDim2.new(0, 8, 0, 0)
        ddIco.BackgroundTransparency = 1
        ddIco.Text = "▣"
        ddIco.Font = Enum.Font.GothamBold
        ddIco.TextSize = 12
        ddIco.TextColor3 = COL.accentHot
        ddIco.ZIndex = 10
        ddIco.Parent = dd

        local search = Instance.new("TextButton")
        search.Size = UDim2.new(0, 28, 0, 28)
        search.Position = UDim2.new(1, -42, 0.5, -14)
        search.BackgroundColor3 = COL.card
        search.BorderSizePixel = 0
        search.Text = "⌕"
        search.Font = Enum.Font.GothamBold
        search.TextSize = 15
        search.TextColor3 = COL.textDim
        search.AutoButtonColor = false
        search.ZIndex = 9
        search.Parent = topBar
        corner(search, 8)
        search.MouseEnter:Connect(function() tween(search, {BackgroundColor3 = COL.cardHover}, 0.12) end)
        search.MouseLeave:Connect(function() tween(search, {BackgroundColor3 = COL.card}, 0.12) end)

        local statusDot = Instance.new("Frame")
        statusDot.Size = UDim2.new(0, 8, 0, 8)
        statusDot.Position = UDim2.new(1, -152, 0.5, -4)
        ...
        dotGlow.Position = UDim2.new(1, -156, 0.5, -8)
        dotGlow.BackgroundColor3 = Color3.fromRGB(80, 220, 120)
        ...
        xdSpawn(function()
            while statusDot.Parent do
                tween(statusDot, {BackgroundTransparency = 0.55}, 0.8, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut)
                tween(dotGlow, {BackgroundTransparency = 0.92}, 0.8)
                xdWait(0.8)
                tween(statusDot, {BackgroundTransparency = 0}, 0.8)
                tween(dotGlow, {BackgroundTransparency = 0.7}, 0.8)
                xdWait(0.8)
            end
        end)
        ...
        local lineGrad = gradient(accentLine, {
            ...
            ColorSequenceKeypoint.new(0.4, COL.accentHot),
            ColorSequenceKeypoint.new(0.6, COL.ember),
            ColorSequenceKeypoint.new(1, COL.accentDim),
        }, 0)
        if lineGrad then
            xdSpawn(function()
                while lineGrad.Parent do
                    tween(lineGrad, {Offset = Vector2.new(0.7, 0)}, 2.6, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut)
                    xdWait(2.6)
                    lineGrad.Offset = Vector2.new(-0.7, 0)
                end
            end)
        end
        ...
        sidebar.Size = UDim2.new(0, 156, 1, -56)
        ...
        tabScroll.Size = UDim2.new(1, 0, 1, -128)
        ...
        local hkHead = Instance.new("TextLabel")
        hkHead.Size = UDim2.new(1, -16, 0, 18)
        hkHead.Position = UDim2.new(0, 8, 1, -116)
        hkHead.BackgroundTransparency = 1
        hkHead.Text = "HOTKEYS"
        hkHead.Font = Enum.Font.GothamBold
        hkHead.TextSize = 10
        hkHead.TextColor3 = COL.textDim
        hkHead.TextXAlignment = Enum.TextXAlignment.Left
        hkHead.ZIndex = 9
        hkHead.Parent = sidebar
        local hk1 = Instance.new("TextLabel")
        hk1.Size = UDim2.new(1, -16, 0, 16)
        hk1.Position = UDim2.new(0, 8, 1, -96)
        hk1.BackgroundTransparency = 1
        hk1.Text = "●  Toggle menu    X"
        hk1.Font = Enum.Font.GothamMedium
        hk1.TextSize = 11
        hk1.TextColor3 = COL.textDim
        hk1.TextXAlignment = Enum.TextXAlignment.Left
        hk1.ZIndex = 9
        hk1.Parent = sidebar
        local hk2 = Instance.new("TextLabel")
        hk2.Size = UDim2.new(1, -16, 0, 16)
        hk2.Position = UDim2.new(0, 8, 1, -78)
        hk2.BackgroundTransparency = 1
        hk2.Text = "●  Farm   F"
        hk2.Font = Enum.Font.GothamMedium
        hk2.TextSize = 11
        hk2.TextColor3 = COL.textDim
        hk2.TextXAlignment = Enum.TextXAlignment.Left
        hk2.ZIndex = 9
        hk2.Parent = sidebar

        local farmBarBg = Instance.new("Frame")
        farmBarBg.Size = UDim2.new(1, -16, 0, 4)
        farmBarBg.Position = UDim2.new(0, 8, 1, -60)
        farmBarBg.BackgroundColor3 = COL.track
        farmBarBg.BorderSizePixel = 0
        farmBarBg.ZIndex = 9
        farmBarBg.Parent = sidebar
        corner(farmBarBg, 2)
        local farmBarFill = Instance.new("Frame")
        farmBarFill.Size = UDim2.new(0, 0, 1, 0)
        farmBarFill.BackgroundColor3 = COL.accent
        farmBarFill.BorderSizePixel = 0
        farmBarFill.ZIndex = 10
        farmBarFill.Parent = farmBarBg
        corner(farmBarFill, 2)

        local prof = Instance.new("Frame")
        prof.Size = UDim2.new(1, -16, 0, 42)
        prof.Position = UDim2.new(0, 8, 1, -50)
        prof.BackgroundColor3 = COL.card
        prof.BorderSizePixel = 0
        prof.ZIndex = 9
        prof.Parent = sidebar
        corner(prof, 9)
        stroke(prof, COL.border, 1, 0.5)
        local profIco = Instance.new("Frame")
        profIco.Size = UDim2.new(0, 28, 0, 28)
        profIco.Position = UDim2.new(0, 7, 0.5, -14)
        profIco.BackgroundColor3 = COL.accentDim
        profIco.BorderSizePixel = 0
        profIco.ZIndex = 10
        profIco.Parent = prof
        corner(profIco, 7)
        local profIcoT = Instance.new("TextLabel")
        profIcoT.Size = UDim2.new(1, 0, 1, 0)
        profIcoT.BackgroundTransparency = 1
        profIcoT.Text = "☠"
        profIcoT.Font = Enum.Font.GothamBold
        profIcoT.TextSize = 14
        profIcoT.TextColor3 = COL.accentHot
        profIcoT.ZIndex = 11
        profIcoT.Parent = profIco
        local profName = Instance.new("TextLabel")
        profName.Size = UDim2.new(1, -56, 0, 17)
        profName.Position = UDim2.new(0, 42, 0, 6)
        profName.BackgroundTransparency = 1
        profName.Text = localplayer.Name
        profName.Font = Enum.Font.GothamBold
        profName.TextSize = 12
        profName.TextColor3 = COL.text
        profName.TextXAlignment = Enum.TextXAlignment.Left
        profName.TextTruncate = Enum.TextTruncate.AtEnd
        profName.ZIndex = 10
        profName.Parent = prof
        local profSub = Instance.new("TextLabel")
        profSub.Size = UDim2.new(1, -56, 0, 13)
        profSub.Position = UDim2.new(0, 42, 0, 22)
        profSub.BackgroundTransparency = 1
        profSub.Text = "never"
        profSub.Font = Enum.Font.GothamMedium
        profSub.TextSize = 10
        profSub.TextColor3 = COL.textDim
        profSub.TextXAlignment = Enum.TextXAlignment.Left
        profSub.ZIndex = 10
        profSub.Parent = prof
        local profArr = Instance.new("TextLabel")
        profArr.Size = UDim2.new(0, 16, 1, 0)
        profArr.Position = UDim2.new(1, -20, 0, 0)
        profArr.BackgroundTransparency = 1
        profArr.Text = "›"
        profArr.Font = Enum.Font.GothamBlack
        profArr.TextSize = 20
        profArr.TextColor3 = COL.textDim
        profArr.ZIndex = 10
        profArr.Parent = prof
        prof.MouseEnter:Connect(function() tween(prof, {BackgroundColor3 = COL.cardHover}, 0.12) end)
        prof.MouseLeave:Connect(function() tween(prof, {BackgroundColor3 = COL.card}, 0.12) end)
        ...
        -- ripple-вспышка по строке при клике
        local function ripple(row)
            pcall(function()
                local r = Instance.new("Frame")
                r.BackgroundColor3 = COL.accentHot
                r.BackgroundTransparency = 0.55
                r.BorderSizePixel = 0
                r.Size = UDim2.new(0, 8, 0, 8)
                r.Position = UDim2.new(0.5, -4, 0.5, -4)
                r.ZIndex = 7
                r.Parent = row
                corner(r, 4)
                tween(r, {Size = UDim2.new(1, 40, 1, 30), Position = UDim2.new(0, -20, 0.5, -15), BackgroundTransparency = 1}, 0.4, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
                xdDelay(0.42, function() r:Destroy() end)
            end)
        end
        ...
        local function rowFrame(parent, order, h)
            ...
            row.BackgroundColor3 = COL.cardHover
        ...
        local function headRow(parent, order, text)
            ...
            dot.BackgroundColor3 = COL.accentHot
            dot.BorderSizePixel = 0
            dot.ZIndex = 9
            dot.Parent = row
            corner(dot, 2)
        ...
        local function labelOf(row, text)
            local l = Instance.new("TextLabel")
        ...
        local function dotsOf(row)
            local d = Instance.new("TextButton")
            d.Size = UDim2.new(0, 24, 1, 0)
            d.Position = UDim2.new(1, -72, 0, 0)
            d.BackgroundTransparency = 1
            d.Text = "···"
            d.Font = Enum.Font.GothamBold
            d.TextSize = 13
            d.TextColor3 = COL.textDim
            d.AutoButtonColor = false
            d.ZIndex = 9
            d.Parent = row
            d.MouseEnter:Connect(function() tween(d, {TextColor3 = COL.accentHot}, 0.1) end)
            d.MouseLeave:Connect(function() tween(d, {TextColor3 = COL.textDim}, 0.1) end)
            return d
        end
        local function switchOf(row, default, cb)
            ...
            local function render()
                if st then
                    tween(sw, {BackgroundColor3 = COL.accent}, 0.16)
                    if swStroke then tween(swStroke, {Color = COL.accentHot, Transparency = 0}, 0.16) end
                    tween(kn, {Position = UDim2.new(1, -19, 0.5, -8)}, 0.22, Enum.EasingStyle.Back, Enum.EasingDirection.Out)
                else
                    tween(sw, {BackgroundColor3 = COL.track}, 0.16)
                    if swStroke then tween(swStroke, {Color = COL.border, Transparency = 0.6}, 0.16) end
                    tween(kn, {Position = UDim2.new(0, 3, 0.5, -8)}, 0.22, Enum.EasingStyle.Back, Enum.EasingDirection.Out)
                end
            end
            render()
            local function set(v, fire)
                ...
                render()
                if fire ~= false then pcall(function() cb(st) end) end
            end
        ...
        local function hoverRow(row)
            row.MouseEnter:Connect(function() tween(row, {BackgroundTransparency = 0.55}, 0.12) end)
            row.MouseLeave:Connect(function() tween(row, {BackgroundTransparency = 1}, 0.12) end)
        end
        local function makeToggle(col, order, text, default, cb)
            local r = rowFrame(col, order, 36)
            hoverRow(r)
            labelOf(r, text)
            dotsOf(r)
            return switchOf(r, default, cb)
        end
        local function makeButton(col, order, text, cb)
            ...
            badge.Position = UDim2.new(1, -46, 0.5, -11)
            badge.BackgroundColor3 = COL.card
            badge.BorderSizePixel = 0
            badge.ZIndex = 10
            badge.Parent = r
            corner(badge, 7)
            stroke(badge, COL.border, 1, 0.5)
            ...
            btn.MouseButton1Click:Connect(function()
                ...
                tween(bt, {TextColor3 = COL.knob}, 0.08)
                xdDelay(0.14, function()
                    tween(badge, {BackgroundColor3 = COL.card}, 0.14)
                    tween(bt, {TextColor3 = COL.textDim}, 0.14)
                end)
                pcall(cb)
            end)
            return btn
        ...
        local function makeInput(col, order, label, default, cb)
            ...
            box.FocusLost:Connect(function()
                tween(box, {BackgroundColor3 = COL.card}, 0.1)
                local v = tonumber(box.Text)
                if v then pcall(function() cb(v) end) else box.Text = tostring(default) end
            end)
            return box
        end
        ...
        local function switchTab(name)
            for n, d in pairs(tabs) do
                d.btn.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
                d.btn.BackgroundTransparency = 1
                d.icon.TextColor3 = COL.textDim
                d.label.TextColor3 = COL.textDim
                d.bar.BackgroundTransparency = 1
            end
            for n, pg in pairs(pages) do pg.Visible = false end
            if tabs[name] then
                local d = tabs[name]
                d.btn.BackgroundColor3 = COL.cardHover
                d.btn.BackgroundTransparency = 0
                d.icon.TextColor3 = COL.accentHot
                d.label.TextColor3 = COL.text
                d.bar.BackgroundTransparency = 0
            end
        ...
        local function makeCol(parent, xScale, xOff)
            ...
            c.Position = UDim2.new(xScale, xOff, 0, 5)
            c.BackgroundTransparency = 1
            c.BorderSizePixel = 0
            c.ScrollBarThickness = 3
            c.ScrollBarImageColor3 = COL.accent
            c.ScrollBarImageTransparency = 0.4
        ...
        local function addTab(name, icon, order)
            ...
            b.MouseEnter:Connect(function()
                if currentTab ~= name then
                    tween(b, {BackgroundColor3 = COL.card, BackgroundTransparency = 0.5}, 0.12)
                    tween(ic, {TextColor3 = COL.text}, 0.12)
                end
            end)
            b.MouseLeave:Connect(function()
                if currentTab ~= name then
                    tween(b, {BackgroundTransparency = 1}, 0.12)
                    tween(ic, {TextColor3 = COL.textDim}, 0.12)
                end
            end)
            ...
            pg.ZIndex = 8
            pg.Parent = content
            local left = makeCol(pg, 0, 2)
            local right = makeCol(pg, 0.5, 6)
            pages[name] = pg
            return left, right
        end
        ...
        headRow(sL, 0, "SHERIFF / HERO")
        headRow(sR, 0, "AUTOMATION")
        makeButton(sL, 1, "Health bar", shootMurderer)
        makeButton(sL, 2, "Name", teleportToGun)
        makeButton(sL, 3, "Avatar", sendNamesToChat)
        makeButton(sL, 4, "Distance", copySheriffName)
        makeButton(sL, 5, "Skeleton", copyMurdererName)
        makeToggle(sR, 1, "Glow chams", false, function(s) autoShooting = s end)
        makeToggle(sR, 2, "Material chams", false, function(s) autoGetDroppedGun = s end)
        makeToggle(sR, 3, "Flags", false, function(s) instakillshoot = s end)
        makeButton(sR, 4, "Gun", function()
            if floatingButtons["TP_GUN"] then removeFloatingButton("TP_GUN")
            else createFloatingButton("TP_GUN", "🔫 К ПУШКЕ", COL.accent, teleportToGun, UDim2.new(0, 120, 0, 80)) end
        end)
        makeButton(sR, 5, "Allow Local", function()
        ...
        headRow(mL, 0, "MURDERER")
        headRow(mR, 0, "AURA / AUTO")
        makeButton(mL, 1, "Backtrack", knifeThrow)
        makeButton(mL, 2, "Self chams", killClosest)
        makeButton(mL, 3, "Tool chams", killEveryone)
        makeButton(mL, 4, "Fake Position", holdHostage)
        makeButton(mL, 5, "God Mode", godMode)
        makeToggle(mR, 1, "Murder", false, function(s) loopThrow = s end)
        makeToggle(mR, 2, "Players Deaths", false, function(s) toggleKillAura(s) end)
        makeToggle(mR, 3, "Bullet tracer", false, function(s) spawnAtPlayer = s end)
        makeToggle(mR, 4, "Ignore knives", false, function(s) ignoreknifethrow = s end)

        local eL, eR = addTab("ESP", "◉", 3)
        headRow(eL, 0, "PLAYERS")
        headRow(eR, 0, "WORLD")
        makeToggle(eL, 1, "Custom fog", false, function(s)
            playerESP = s
            if s then ensureEspWatcher(); notify(HUB_NAME, "ESP включён") end
            refreshESP()
        end)
        makeToggle(eL, 2, "Fullbright", false, function(s) hideMeEsp = s; refreshESP() end)
        makeToggle(eR, 1, "Custom ambient", false, function(s) gunDropESP = s; reloadGunESP() end)
        makeToggle(eR, 2, "Shaders", false, function(s) trapDetection = s; reloadTrapESP() end)

        local pL, pR = addTab("Игрок", "◎", 4)
        headRow(pL, 0, "TELEPORT")
        headRow(pR, 0, "SETTINGS")
        makeButton(pL, 1, "Aura", teleportToLobby)
        makeButton(pL, 2, "Time changer", teleportToMap)
        makeInput(pR, 1, "Exposure changer", shootOffset, function(v) shootOffset = v end)
        makeInput(pR, 2, "Skybox", offsetToPingMult, function(v) offsetToPingMult = v end)

        local fL, fR = addTab("Фарм", "⚙", 5)
        headRow(fL, 0, "FARM")
        headRow(fR, 0, "STATS")
        makeToggle(fL, 1, "World effects", false, function(s)
            isActive = s
            if s then startFarming() else farmStopped = true end
        end)
        makeToggle(fL, 2, "Crosshair", false, function(s) antiAFK = s end)
        makeToggle(fL, 3, "Fling on full", false, function(s) flingOnFullBag = s end)
        makeInput(fL, 4, "Fly speed", flySpeed, function(v) flySpeed = clamp(v, 4, 60) end)
        makeInput(fL, 5, "Bag size", bagSize, function(v) bagSize = clamp(math.floor(v), 1, 999) end)
        makeButton(fL, 6, "Fling murder", function()
            local m = findMurderer()
            if not m then notify(HUB_NAME, "Нет убийцы."); return end
            miniFling(m)
        end)
        makeButton(fL, 7, "Fling sheriff", function()
            local s = findSheriff()
            if not s then notify(HUB_NAME, "Нет шерифа."); return end
            miniFling(s)
        end)
        local roleV = makeStat(fR, 1, "Role")
        local counterV = makeStat(fR, 2, "Coins")
        local timerV = makeStat(fR, 3, "Time")
        local rateV = makeStat(fR, 4, "Rate")
        local pCoinV = makeStat(fR, 5, "Total")
        local bagVal = makeStat(fR, 6, "Bag")

        local vL, vR = addTab("Визуал", "✦", 6)
        headRow(vL, 0, "EFFECTS")
        headRow(vR, 0, "AURA / WORLD")
        local visualToggles = {}
        visualToggles.wings = makeToggle(vL, 1, "Wings", true, function(s)
            visualState.wings = s
            if s then applyVisualSafe("wings") else clearVisual("wings") end
        end)
        visualToggles.halo = makeToggle(vL, 2, "Halo orb", true, function(s)
            visualState.halo = s
            if s then applyVisualSafe("halo") else clearVisual("halo") end
        end)
        visualToggles.circle = makeToggle(vL, 3, "Floor ripple", true, function(s)
            visualState.circle = s
            if s then applyVisualSafe("circle") else clearVisual("circle") end
        end)
        visualToggles.trails = makeToggle(vL, 4, "Neon trails", false, function(s)
            visualState.trails = s
            if s then applyVisualSafe("trails") else clearVisual("trails") end
        end)
        visualToggles.eyes = makeToggle(vL, 5, "Glow eyes", false, function(s)
            visualState.eyes = s
            if s then applyVisualSafe("eyes") else clearVisual("eyes") end
        end)
        visualToggles.aura = makeToggle(vR, 1, "Red aura", false, function(s)
            visualState.aura = s
            if s then applyVisualSafe("aura") else clearVisual("aura") end
        end)
        visualToggles.fire = makeToggle(vR, 2, "Fire aura", false, function(s)
            visualState.fire = s
            if s then applyVisualSafe("fire") else clearVisual("fire") end
        end)
        visualToggles.smoke = makeToggle(vR, 3, "Dark smoke", false, function(s)
            visualState.smoke = s
            if s then applyVisualSafe("smoke") else clearVisual("smoke") end
        end)
        visualToggles.lightning = makeToggle(vR, 4, "Crimson bolts", false, function(s)
            visualState.lightning = s
            if s then applyVisualSafe("lightning") else clearVisual("lightning") end
        end)
        visualToggles.light = makeToggle(vR, 5, "Red light", false, function(s)
            visualState.light = s
            if s then applyVisualSafe("light") else clearVisual("light") end
        end)
        makeButton(vR, 6, "Enable all", function()
            for _, t in pairs(visualToggles) do t:Set(true) end
            notify(HUB_NAME, "Все эффекты включены!")
        end)
        makeButton(vR, 7, "Disable all", function()
        ...
        function updateRoleUI()
            checkRole()
            local rn, rc
            if isMurderer then rn = "Murderer"; rc = Color3.fromRGB(255, 70, 80)
            elseif isSheriff then rn = "Sheriff"; rc = Color3.fromRGB(90, 160, 255)
            elseif isHero then rn = "Hero"; rc = Color3.fromRGB(255, 200, 96)
            else rn = "Innocent"; rc = Color3.fromRGB(90, 220, 120) end
            if roleV then roleV.Text = rn; roleV.TextColor3 = rc end
        end
        function updateBagUI()
            local cc = getCollectedCoins()
            if farmStopped then
                bagVal.Text = "stop"; bagVal.TextColor3 = Color3.fromRGB(255, 80, 80)
            elseif cc >= bagSize then
                bagVal.Text = "full"; bagVal.TextColor3 = Color3.fromRGB(255, 200, 0)
            else
                bagVal.Text = cc .. "/" .. bagSize; bagVal.TextColor3 = COL.accentHot
            end
            pcall(function()
                local f = math.min(1, cc / math.max(1, bagSize))
                tween(farmBarFill, {Size = UDim2.new(f, 0, 1, 0)}, 0.3)
            end)
        end
                ...
                while isActive do
                    ...
                    if cc >= bagSize then
                        ...
                        if not bagFullNotified then
                            ...
                            notify(HUB_NAME, "Мешок полный (" .. cc .. "/" .. bagSize .. ")")
                        end
                        updateBagUI()
                        xdWait(1)
                        continue
        ...
        xdSpawn(function()
            while mBtn.Parent do
                tween(mBtn, {Size = UDim2.new(0, 55, 0, 55)}, 1.3, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut)
                xdWait(1.3)
                tween(mBtn, {Size = UDim2.new(0, 50, 0, 50)}, 1.3, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut)
                xdWait(1.3)
            end
        end)
        ...
        notify(HUB_NAME, "v42 loaded")
        notify(HUB_NAME, "wings + halo orb + floor ripple")
    end)

    if not guiOK then
        ban(HUB_NAME .. " — GUI build error:\n" .. tostring(guiERR), Color3.fromRGB(255, 90, 90))
        warn("[GUI BUILD ERROR] " .. tostring(guiERR))
    else
        ban("[" .. HUB_NAME .. "] READY ✓", Color3.fromRGB(90, 255, 130))
    end
    xdStatus(HUB_NAME .. " v42: ready", Color3.fromRGB(80, 255, 120))
