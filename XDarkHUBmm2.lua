...
xpcall(function()
    ...
    local visualObjects = {}
    local wingParts = {}; local wingMats = {}
    local haloBeads = {}; local haloRoot = nil
    local floorWaves = {}; local floorPool = nil; local floorPoolGlow = nil
    local eyeParts = {}
    ...
    local function clearVisual(name) if visualObjects[name] then for _, obj in ipairs(visualObjects[name]) do pcall(function() obj:Destroy() end) end; visualObjects[name] = nil end; if name == "wings" then wingParts = {}; wingMats = {} end; if name == "halo" then haloBeads = {}; haloRoot = nil end; if name == "circle" then floorWaves = {}; floorPool = nil; floorPoolGlow = nil end; if name == "eyes" then eyeParts = {} end end
    ...
    local function glowOf(src, scale, transp, color)
        local g = nil
        pcall(function()
            g = src:Clone(); g.Name = src.Name .. "_g"; g.Transparency = transp or 0.55; if color then g.Color = color end
            if g:FindFirstChildOfClass("SpecialMesh") then g:FindFirstChildOfClass("SpecialMesh").Scale = src:FindFirstChildOfClass("SpecialMesh").Scale * (scale or 1.5) else g.Size = src.Size * (scale or 1.5) end
            g.Parent = src.Parent
        end)
        return g
    end
    ...
    local function applyWings()
        ...
        for side = -1, 1, 2 do
            ...
            registerVisual("wings", mat); if matG then registerVisual("wings", matG) end
            ...
            for i = 1, count do
                ...
                table.insert(wingParts, {part = feather, glow = fG, side = side, i = i, len = len, baseAng = 10 + t * 78, flapAmp = 5 + t * 14})
            end
        end
        local att = Instance.new("Attachment", hrp); att.Position = Vector3.new(0, 1.1, 0.3); registerVisual("wings", att)
        local em = Instance.new("ParticleEmitter", att); em.Texture = "rbxasset://textures/particles/sparkles_main.dds"
        em.Color = ColorSequence.new(CORE, GLOW); em.Rate = 42; em.Lifetime = NumberRange.new(0.6, 1.3); em.Speed = NumberRange.new(1, 3)
        em.SpreadAngle = Vector2.new(180, 180); em.LightEmission = 1; em.Size = NumberSequence.new({NumberSequenceKeypoint.new(0, 0.42), NumberSequenceKeypoint.new(1, 0)}); registerVisual("wings", em)
        local wl = Instance.new("PointLight"); wl.Color = Color3.fromRGB(255, 80, 95); wl.Brightness = 3.4; wl.Range = 28; wl.Parent = hrp; registerVisual("wings", wl)
    end
    ...
    local function applyHalo()
        ...
        haloRoot = Instance.new("Part"); haloRoot.Name = "XHaloRoot"; haloRoot.Transparency = 1; haloRoot.Anchored = true; haloRoot.CanCollide = false; haloRoot.Size = Vector3.new(0.1, 0.1, 0.1); haloRoot.Parent = char; registerVisual("halo", haloRoot)
        local beads = 32; local rad = 1.55
        for k = 1, beads do
            ...
            registerVisual("halo", bead); table.insert(haloBeads, {part = bead, a = a, rad = rad})
        end
        local hl = Instance.new("PointLight"); hl.Color = Color3.fromRGB(255, 80, 95); hl.Brightness = 2.8; hl.Range = 15; hl.Parent = head; registerVisual("halo", hl)
    end
    ...
    local function applyCircle()
        ...
        floorPool = neon({Name = "XPool", Shape = Enum.PartType.Cylinder, Size = Vector3.new(0.1, 9, 9), Color = GLOW, Transparency = 0.82, Parent = char}); registerVisual("circle", floorPool)
        floorPoolGlow = glowOf(floorPool, 1.12, 0.9, GLOW); if floorPoolGlow then registerVisual("circle", floorPoolGlow) end
        for i = 1, 4 do
            ...
            registerVisual("circle", disc); if dG then registerVisual("circle", dG) end
            table.insert(floorWaves, {part = disc, glow = dG, phase = (i - 1) / 4})
        end
        local cl = Instance.new("PointLight"); cl.Color = Color3.fromRGB(255, 70, 85); cl.Brightness = 3.0; cl.Range = 24; cl.Parent = hrp; registerVisual("circle", cl)
    end
    ...
    RunService.Heartbeat:Connect(function()
        pcall(function()
            ...
            local hrp = char:FindFirstChild("HumanoidRootPart"); local t = tick()
            if visualState.wings and hrp then
                ...
                for _, f in ipairs(wingParts) do
                    if f.part.Parent then
                        ...
                        local cf = hrp.CFrame * CFrame.new(f.side * 0.35, 1.2 + bob, -0.4) * CFrame.Angles(0, 0, math.rad(f.side * a)) * CFrame.new(0, f.len / 2, 0)
                ...
                for _, m in ipairs(wingMats) do
                    if m.part.Parent then
                        ...
                        local cf = hrp.CFrame * CFrame.new(m.side * 1.05, 1.15 + bob, 0.9) * CFrame.Angles(0, 0, math.rad(m.side * spread)) * CFrame.Angles(0, math.rad(m.side * -8), 0)
            ...
            if visualState.halo and haloRoot and haloRoot.Parent then
                ...
                if head then
                    ...
                    local rootCF = head.CFrame * CFrame.new(0, 1.95 + bob, 0) * CFrame.Angles(math.rad(70), t * 1.2, math.rad(10))
                    haloRoot.CFrame = rootCF
                    for _, b in ipairs(haloBeads) do
                        if b.part.Parent then
                            local lx = math.cos(b.a) * b.rad; local lz = math.sin(b.a) * b.rad
            ...
            if visualState.circle and hrp then
                local cx, cz = hrp.Position.X, hrp.Position.Z; local cy = hrp.Position.Y - 2.95
                local poolCF = CFrame.new(cx, cy, cz) * CFrame.Angles(math.rad(90), 0, 0)
                if floorPool and floorPool.Parent then floorPool.CFrame = poolCF; floorPool.Transparency = 0.8 + math.sin(t * 1.5) * 0.05 end
                ...
                for _, w in ipairs(floorWaves) do
                    if w.part.Parent then
                        ...
                        w.part.CFrame = cf; w.part.Size = Vector3.new(0.12, r * 2, r * 2); w.part.Transparency = tr
                        if w.glow and w.glow.Parent then w.glow.CFrame = cf; w.glow.Size = w.part.Size * 1.1; w.glow.Transparency = math.min(0.95, tr + 0.2) end
                    end
                end
            end
    ...
    local guiOK, guiERR = pcall(function()
        ...
        local hk1 = Instance.new("TextLabel"); hk1.Size = UDim2.new(1, -16, 0, 16); hk1.Position = UDim2.new(0, 8, 1, -96); hk1.BackgroundTransparency = 1; hk1.Text = "●  Toggle menu    X"; hk1.Font = Enum.Font.GothamMedium; hk1.TextSize = 11; hk1.TextColor3 = COL.textDim; hk1.TextXAlignment = Enum.TextXAlignment.Left; hk1.ZIndex = 9; hk1.Parent = sidebar
        local hk2 = Instance.new("TextLabel"); hk2.Size = UDim2.new(1, -16, 0, 16); hk2.Position = UDim2.new(0, 8, 1, -78); hk2.BackgroundTransparency = 1; hk2.Text = "●  Farm   F"; hk2.Font = Enum.Font.GothamMedium; hk2.TextSize = 11; hk2.TextColor3 = COL.textDim; hk2.TextXAlignment = Enum.TextXAlignment.Left; hk2.ZIndex = 9; hk2.Parent = sidebar
        ...
        local function rowFrame(parent, order, h)
            local row = Instance.new("Frame"); row.Size = UDim2.new(1, 0, 0, h or 36); row.BackgroundColor3 = COL.cardHover; row.BackgroundTransparency = 1; row.LayoutOrder = order; row.ZIndex = 8; row.Parent = parent; corner(row, 6)
            return row
        end
        ...
        local function labelOf(row, text)
            local l = Instance.new("TextLabel"); l.Size = UDim2.new(1, -86, 1, 0); l.Position = UDim2.new(0, 8, 0, 0); l.BackgroundTransparency = 1; l.Text = text; l.Font = Enum.Font.GothamMedium; l.TextSize = 13; l.TextColor3 = COL.text; l.TextXAlignment = Enum.TextXAlignment.Left; l.ZIndex = 9; l.Parent = row
            return l
        end
        local function dotsOf(row)
            local d = Instance.new("TextButton"); d.Size = UDim2.new(0, 24, 1, 0); d.Position = UDim2.new(1, -72, 0, 0); d.BackgroundTransparency = 1; d.Text = "···"; d.Font = Enum.Font.GothamBold; d.TextSize = 13; d.TextColor3 = COL.textDim; d.AutoButtonColor = false; d.ZIndex = 9; d.Parent = row
            d.MouseEnter:Connect(function() tween(d, {TextColor3 = COL.accentHot}, 0.1) end); d.MouseLeave:Connect(function() tween(d, {TextColor3 = COL.textDim}, 0.1) end)
            return d
        end
        local function switchOf(row, default, cb)
            ...
            sw.MouseButton1Click:Connect(function() playClick(); set(not st) end)
        ...
        local function makeButton(col, order, text, cb)
            ...
            btn.MouseButton1Click:Connect(function() playClick(); tween(badge, {BackgroundColor3 = COL.accent}, 0.08); tween(bt, {TextColor3 = COL.knob}, 0.08); xdDelay(0.14, function() tween(badge, {BackgroundColor3 = COL.card}, 0.14); tween(bt, {TextColor3 = COL.textDim}, 0.14) end); pcall(cb) end)
            return btn
        end
        ...
        local function makeStat(col, order, label)
            local r = rowFrame(col, order, 34); local l = Instance.new("TextLabel"); l.Size = UDim2.new(0.55, -8, 1, 0); l.Position = UDim2.new(0, 8, 0, 0); l.BackgroundTransparency = 1; l.Text = label; l.Font = Enum.Font.GothamMedium; l.TextSize = 12; l.TextColor3 = COL.textDim; l.TextXAlignment = Enum.TextXAlignment.Left; l.ZIndex = 9; l.Parent = r
        ...
        headRow(sL, 0, "SHERIFF / HERO"); headRow(sR, 0, "AUTOMATION")
        makeButton(sL, 1, "Health bar", shootMurderer); makeButton(sL, 2, "Name", teleportToGun); makeButton(sL, 3, "Avatar", sendNamesToChat); makeButton(sL, 4, "Distance", copySheriffName); makeButton(sL, 5, "Skeleton", copyMurdererName)
        makeToggle(sR, 1, "Glow chams", false, function(s) autoShooting = s end); makeToggle(sR, 2, "Material chams", false, function(s) autoGetDroppedGun = s end); makeToggle(sR, 3, "Flags", false, function(s) instakillshoot = s end)
        makeButton(sR, 4, "Gun", function() if floatingButtons["TP_GUN"] then removeFloatingButton("TP_GUN") else createFloatingButton("TP_GUN", "🔫 К ПУШКЕ", COL.accent, teleportToGun, UDim2.new(0, 120, 0, 80)) end end)
        makeButton(sR, 5, "Allow Local", function() if floatingButtons["SHOOT"] then removeFloatingButton("SHOOT") else createFloatingButton("SHOOT", "🔫 ВЫСТРЕЛ", COL.accent, shootMurderer, UDim2.new(0, 120, 0, 145)) end end)

        local mL, mR = addTab("Убийца", "☠", 2)
        headRow(mL, 0, "MURDERER"); headRow(mR, 0, "AURA / AUTO")
        makeButton(mL, 1, "Backtrack", knifeThrow); makeButton(mL, 2, "Self chams", killClosest); makeButton(mL, 3, "Tool chams", killEveryone); makeButton(mL, 4, "Fake Position", holdHostage); makeButton(mL, 5, "God Mode", godMode)
        makeToggle(mR, 1, "Murder", false, function(s) loopThrow = s end); makeToggle(mR, 2, "Players Deaths", false, function(s) toggleKillAura(s) end); makeToggle(mR, 3, "Bullet tracer", false, function(s) spawnAtPlayer = s end); makeToggle(mR, 4, "Ignore knives", false, function(s) ignoreknifethrow = s end)

        local eL, eR = addTab("ESP", "◉", 3)
        headRow(eL, 0, "PLAYERS"); headRow(eR, 0, "WORLD")
        makeToggle(eL, 1, "Custom fog", false, function(s) playerESP = s; if s then ensureEspWatcher(); notify(HUB_NAME, "ESP включён") end; refreshESP() end); makeToggle(eL, 2, "Fullbright", false, function(s) hideMeEsp = s; refreshESP() end)
        makeToggle(eR, 1, "Custom ambient", false, function(s) gunDropESP = s; reloadGunESP() end); makeToggle(eR, 2, "Shaders", false, function(s) trapDetection = s; reloadTrapESP() end)

        local pL, pR = addTab("Игрок", "◎", 4)
        headRow(pL, 0, "TELEPORT"); headRow(pR, 0, "SETTINGS")
        makeButton(pL, 1, "Aura", teleportToLobby); makeButton(pL, 2, "Time changer", teleportToMap)
        makeInput(pR, 1, "Exposure changer", shootOffset, function(v) shootOffset = v end); makeInput(pR, 2, "Skybox", offsetToPingMult, function(v) offsetToPingMult = v end)

        local fL, fR = addTab("Фарм", "⚙", 5)
        headRow(fL, 0, "FARM"); headRow(fR, 0, "STATS")
        makeToggle(fL, 1, "World effects", false, function(s) isActive = s; if s then startFarming() else farmStopped = true end end); makeToggle(fL, 2, "Crosshair", false, function(s) antiAFK = s end); makeToggle(fL, 3, "Fling on full", false, function(s) flingOnFullBag = s end)
        makeInput(fL, 4, "Fly speed", flySpeed, function(v) flySpeed = clamp(v, 4, 60) end); makeInput(fL, 5, "Bag size", bagSize, function(v) bagSize = clamp(math.floor(v), 1, 999) end)
        makeButton(fL, 6, "Fling murder", function() local m = findMurderer(); if not m then notify(HUB_NAME, "Нет убийцы."); return end; miniFling(m) end); makeButton(fL, 7, "Fling sheriff", function() local s = findSheriff(); if not s then notify(HUB_NAME, "Нет шерифа."); return end; miniFling(s) end)
        local roleV = makeStat(fR, 1, "Role"); local counterV = makeStat(fR, 2, "Coins"); local timerV = makeStat(fR, 3, "Time"); local rateV = makeStat(fR, 4, "Rate"); local pCoinV = makeStat(fR, 5, "Total"); local bagVal = makeStat(fR, 6, "Bag")

        local vL, vR = addTab("Визуал", "✦", 6)
        headRow(vL, 0, "EFFECTS"); headRow(vR, 0, "AURA / WORLD")
        local visualToggles = {}
        visualToggles.wings = makeToggle(vL, 1, "Wings", true, function(s) visualState.wings = s; if s then applyVisualSafe("wings") else clearVisual("wings") end end)
        visualToggles.halo = makeToggle(vL, 2, "Halo orb", true, function(s) visualState.halo = s; if s then applyVisualSafe("halo") else clearVisual("halo") end end)
        visualToggles.circle = makeToggle(vL, 3, "Floor ripple", true, function(s) visualState.circle = s; if s then applyVisualSafe("circle") else clearVisual("circle") end end)
        visualToggles.trails = makeToggle(vL, 4, "Neon trails", false, function(s) visualState.trails = s; if s then applyVisualSafe("trails") else clearVisual("trails") end end)
        visualToggles.eyes = makeToggle(vL, 5, "Glow eyes", false, function(s) visualState.eyes = s; if s then applyVisualSafe("eyes") else clearVisual("eyes") end end)
        visualToggles.aura = makeToggle(vR, 1, "Red aura", false, function(s) visualState.aura = s; if s then applyVisualSafe("aura") else clearVisual("aura") end end)
        visualToggles.fire = makeToggle(vR, 2, "Fire aura", false, function(s) visualState.fire = s; if s then applyVisualSafe("fire") else clearVisual("fire") end end)
        visualToggles.smoke = makeToggle(vR, 3, "Dark smoke", false, function(s) visualState.smoke = s; if s then applyVisualSafe("smoke") else clearVisual("smoke") end end)
        visualToggles.lightning = makeToggle(vR, 4, "Crimson bolts", false, function(s) visualState.lightning = s; if s then applyVisualSafe("lightning") else clearVisual("lightning") end end)
        visualToggles.light = makeToggle(vR, 5, "Red light", false, function(s) visualState.light = s; if s then applyVisualSafe("light") else clearVisual("light") end end)
        makeButton(vR, 6, "Enable all", function() for _, t in pairs(visualToggles) do t:Set(true) end; notify(HUB_NAME, "Все эффекты включены!") end)
        makeButton(vR, 7, "Disable all", function() for _, t in pairs(visualToggles) do t:Set(false) end; notify(HUB_NAME, "Все эффекты выключены!") end)
        ...
        function updateRoleUI() checkRole(); local rn, rc; if isMurderer then rn = "Murderer"; rc = Color3.fromRGB(255, 70, 80) elseif isSheriff then rn = "Sheriff"; rc = Color3.fromRGB(90, 160, 255) elseif isHero then rn = "Hero"; rc = Color3.fromRGB(255, 200, 96) else rn = "Innocent"; rc = Color3.fromRGB(90, 220, 120) end; if roleV then roleV.Text = rn; roleV.TextColor3 = rc end end
        function updateBagUI() local cc = getCollectedCoins(); if farmStopped then bagVal.Text = "stop"; bagVal.TextColor3 = Color3.fromRGB(255, 80, 80) elseif cc >= bagSize then bagVal.Text = "full"; bagVal.TextColor3 = Color3.fromRGB(255, 200, 0) else bagVal.Text = cc .. "/" .. bagSize; bagVal.TextColor3 = COL.accentHot end; pcall(function() local f = math.min(1, cc / math.max(1, bagSize)); tween(farmBarFill, {Size = UDim2.new(f, 0, 1, 0)}, 0.3) end) end
        ...
        notify(HUB_NAME, "v42 loaded"); notify(HUB_NAME, "wings + halo orb + floor ripple")
    end)
